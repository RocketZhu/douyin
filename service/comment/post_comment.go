...
func (p *PostCommentFlow) CreateComment() (*models2.Comment, error) {
	...
	err := dao.NewCommentDAO().AddCommentAndUpdateCount(&comment)
	if err != nil {
		return nil, err
	}
...
func (p *PostCommentFlow) DeleteComment() (*models2.Comment, error) {
	...
	err := dao.NewCommentDAO().QueryCommentById(p.commentId, &comment)
	if err != nil {
		return nil, err
	}
	//删除comment
	err = dao.NewCommentDAO().DeleteCommentAndUpdateCountById(p.commentId, p.videoId)
	if err != nil {
		return nil, err
	}
	return &comment, nil
...
func (p *PostCommentFlow) checkNum() error {
	...
	if !dao.NewVideoDAO().IsVideoExistById(p.videoId) {
...
func (p *PostCommentFlow) packData() error {
	...
	_ = dao.NewUserInfoDAO().QueryUserInfoById(p.comment.UserInfoId, &userInfo)
