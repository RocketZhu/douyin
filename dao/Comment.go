...
func (c *CommentDAO) AddCommentAndUpdateCount(comment *models.Comment) error {
	if comment == nil {
...
func (c *CommentDAO) QueryCommentById(id int64, comment *models.Comment) error {
	if comment == nil {
...
func (c *CommentDAO) QueryCommentListByVideoId(videoId int64, comments *[]*models.Comment) error {
	if comments == nil {
		return errors.New("QueryCommentListByVideoId comments空指针")
	}
	if err := DB.Model(&models.Comment{}).Where("video_id=?", videoId).Find(comments).Error; err != nil {
		return err
	}
	return nil
}
