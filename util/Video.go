...
func FillVideoListFields(userId int64, videos *[]*models.Video) (*time.Time, error) {
	...
	for i := 0; i < size; i++ {
		...
		err := infoDAO.QueryUserInfoById((*videos)[i].UserInfoId, &userInfo)
		if err != nil {
			continue
		}
		...
		//填充有登录信息的点赞状态
		if userId > 0 {
			(*videos)[i].IsFavorite = p.GetFavorState(userId, (*videos)[i].Id)
		}
	}
...
func SaveImageFromVideo(name string, isDebug bool) error {
	...
	v2i.InputPath = filepath.Join(config.ServerConfig.StaticSourcePath, name+defaultVideoSuffix)
	v2i.OutputPath = filepath.Join(config.ServerConfig.StaticSourcePath, name+defaultImageSuffix)
	v2i.FrameCount = 1
